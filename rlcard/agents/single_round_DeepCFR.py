...
class DeepMCCFR():
    ...
    def train(self):
        ...
        # for p in range(self._num_players):
        #     print("start current player")
            # while init_player != p:
            #     init_state, init_player = self._env.init_game()
            #     self._root_node = init_state
            # print("get one round start")
            for _ in range(self._num_traversals):
                self._traverse_game_tree(self._root_node, init_player, my_reach=1.0, opp_reach=1.0, sample_reach=1.0)
            # print("get one round done")
            # Re-initialize advantage networks and train from scratch.
            self.reinitialize_advantage_networks()
            # print("reinitialize_advantage_networks")
            # print("_learn_advantage_network start")
            for _ in range(self._num_step):
                self.advantage_losses[0].append(self._learn_advantage_network(0))
                # print("step", _)
            # print("_learn_advantage_network done")
            # Re-initialize advantage networks and train from scratch.
            self._iteration += 1
        # print()
        ...
        print("policy_loss : ",policy_loss, end='')
    ...
    def _traverse_game_tree(self, state, player, my_reach, opp_reach, sample_reach):
        ...
        # info_state_key, actions = self.get_state(current_player)
        ...
        if current_player == player:
            ...
            for action in actions:
                strategy[action] = self._expl / len(actions) + (1 - self._expl) * strategy[action]
            probs = np.array(strategy)
            probs /= probs.sum()

            sampled_action = np.random.choice(np.arange(len(strategy)), p=probs)
            new_my_reach = my_reach * probs[sampled_action]
            new_opp_reach = opp_reach
            new_sample_reach = sample_reach * probs[sampled_action]

            child_state, _ = self._env.step(sampled_action)

            expected_payoff, reach_tail = self._traverse_game_tree(child_state, player, new_my_reach, new_opp_reach, new_sample_reach)

            new_reach_tail = probs[sampled_action] * reach_tail
            # for action in actions:
            ...
            w = expected_payoff[player] * opp_reach
            for action in actions:
                if (action == sampled_action):
                    regret = w * (reach_tail - new_reach_tail)
                    sampled_regret[action] = regret
                else:
            ...
            return expected_payoff, new_reach_tail
        else:
            ...
            new_opp_reach = opp_reach * probs[action]
            new_sample_reach = sample_reach * probs[action]
            child_state, _ = self._env.step(action)
            ...
            return self._traverse_game_tree(child_state, player, new_my_reach,
