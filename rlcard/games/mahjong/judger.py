...
class MahjongJudger(object):
    ...
    def judge_pong_gong(dealer, players, last_player):
        ...
        for player in players:
            ...
            if hand.count(last_card_str) == 3:
                act_list.append('gong')
                player_list.append(player)
                cards_list.append([last_card]*4)
                # return ['gong'], player, [last_card]*4
            # check pong
            if hand.count(last_card_str) == 2:
                act_list.append('pong')
                player_list.append(player)
                cards_list.append([last_card]*4)
                # return ['pong'], player, [last_card]*3
        return act_list, player_list, cards_list
        # return [], None, None
    ...
    def judge_chow(self, dealer, players, last_player):
        ...
        for player in players:
            ...
            if last_card_type != "dragons" and last_card_type != "winds" and last_player == (player.get_player_id() - 1) %  len(players):
                #flag = False
