...
class MahjongRound(object):
    ...
    def proceed_round(self, players, action):
        ...
        if action == 'stand':
            ...
            players[self.current_player].gong(self.dealer, self.last_cards[self.valid_act.index('gong')])
            self.last_player = self.current_player
            self.valid_act = False

        elif action == 'pong':
            players[self.current_player].pong(self.dealer, self.last_cards[self.valid_act.index('pong')])
            self.last_player = self.current_player
            self.valid_act = False

        elif action == 'chow':
            players[self.current_player].chow(self.dealer, self.last_cards[self.valid_act.index('chow')])
            self.last_player = self.current_player
            self.valid_act = False
